...
export async function renderProducts() {
  ...
  pageContent.innerHTML = `
    ...
    <div class="card mb-2" style="padding:10px;">
      <input type="search" id="product-search" class="form-control" placeholder="ค้นหาชื่อ / barcode / SKU"
    ...
    <div id="product-list">
      ${products.length === 0 ? `
        <div class="card text-center text-muted">
          <p>ยังไม่มีสินค้า</p>
          <button class="btn btn-primary btn-sm mt-1" id="btn-add-empty">เพิ่มสินค้าแรก</button>
        </div>
      ` : products.map(p => {
        const stock = Number(p.stock) || 0;
        const low = stock > 0 && stock <= (Number(p.minStock) || 5);
        return `
          <div class="card" data-id="${escapeHtml(p.id)}" style="margin-bottom:8px;padding:12px;cursor:pointer;">
            <div class="flex-between">
              <div style="flex:1;min-width:0;">
                ...
                <div class="text-muted" style="font-size:0.8rem;">
                  ${p.barcode ? escapeHtml(p.barcode) + ' · ' : ''}${escapeHtml(catMap[p.categoryId] || '-')}
                </div>
              </div>
              <div style="text-align:right;">
                <div style="font-weight:700;color:var(--primary);">฿${formatMoney(p.sellPrice)}</div>
                <div style="font-size:0.8rem;color:${stock <= 0 ? 'var(--danger)' : low ? '#b45309' : 'inherit'};">
                  คงเหลือ ${stock}
                </div>
              </div>
            </div>
...
export async function openProductDetail(productId) {
  ...
  pageContent.innerHTML = `
    <div class="flex-between mb-2">
      <h2 style="font-size:1.1rem;">${escapeHtml(product.name)}</h2>
      <button class="btn btn-outline btn-sm" id="btn-back-products">กลับ</button>
    </div>
    <div class="card mb-2">
      ${product.imageUrl ? `<img src="${escapeHtml(product.imageUrl)}" style="max-width:100%;border-radius:8px;margin-bottom:12px;">` : ''}
      <p class="text-muted" style="font-size:0.85rem;">
        SKU: ${escapeHtml(product.sku || '-')}<br>
        Barcode: ${escapeHtml(product.barcode || '-')}<br>
        หน่วย: ${escapeHtml(product.unit || 'ชิ้น')}<br>
        สถานะ: ${productStatusLabel(product.status)}
      </p>
      <div class="grid-2" style="gap:8px;margin-top:10px;">
        ...
        <div class="stat-box"><div class="stat-label">แจ้งเตือนเมื่อ</div><div class="stat-value">≤ ${product.minStock ?? 5}</div></div>
      </div>
    </div>
...
export async function openProductForm(productId) {
      ...
      <form id="product-form">
        ...
        <div class="form-group">
          ...
          <select id="p-category" class="form-control">
            ...
            ${categories.filter(c => c.status !== 'INACTIVE').map(c => `
              <option value="${escapeHtml(c.id)}" ${product?.categoryId === c.id ? 'selected' : ''}>${escapeHtml(c.name)}</option>
            `).join('')}
          </select>
        </div>
        <div class="form-group">
        ...
        <div class="grid-2" style="gap:10px;">
          <div class="form-group">
            ...
            <input type="number" id="p-cost" class="form-control" value="${product?.costPrice != null ? product.costPrice : ''}" min="0" step="0.01" required>
          </div>
          <div class="form-group">
            <label>ราคาขาย (บาท) *</label>
            <input type="number" id="p-sell" class="form-control" value="${product?.sellPrice != null ? product.sellPrice : ''}" min="0" step="0.01" required>
          </div>
        </div>
        <div class="grid-2" style="gap:10px;">
          <div class="form-group">
            <label>จำนวน Stock เริ่มต้น</label>
            <input type="number" id="p-stock" class="form-control" value="${product ? (product.stock != null ? product.stock : 0) : 0}" min="0" step="1" ${product ? 'readonly' : ''}>
          </div>
          <div class="form-group">
        ...
        <div class="form-group">
          ...
          <input type="number" id="p-min" class="form-control" value="${product?.minStock != null ? product.minStock : 5}" min="0" step="1">
        </div>
        <div class="form-group">
          <label>สถานะ</label>
  ...
  $('#product-form')?.addEventListener('submit', async (e) => {
    e.preventDefault();
    await saveProductForm(productId);
  });
  $('#btn-scan-product-bc')?.addEventListener('click', async () => {
    showToast('ใช้พิมพ์ barcode ในช่องด้านบนได้', 'info');
  });
}
...
export async function openStockInForm(productId) {
  ...
  pageContent.innerHTML = `
    <div class="card">
      <h2 style="font-size:1.1rem;">รับสินค้าเข้า</h2>
      <p class="text-muted">${escapeHtml(product.name)} · คงเหลือ ${product.stock || 0}</p>
      <div class="form-group"><label>จำนวน *</label>
        <input type="number" id="si-qty" class="form-control" min="1" step="1" value="1" inputmode="numeric">
      </div>
      <div class="form-group"><label>ราคาทุนต่อหน่วย</label>
        <input type="number" id="si-cost" class="form-control" min="0" step="0.01" value="${product.costPrice != null ? product.costPrice : ''}">
      </div>
      <div class="form-group"><label>หมายเหตุ</label>
        <input type="text" id="si-note" class="form-control">
      </div>
      <button class="btn btn-primary btn-block" id="btn-si-save">บันทึกรับเข้า</button>
      <button class="btn btn-outline btn-block mt-1" id="btn-si-back">กลับ</button>
    </div>`;
  ...
  $('#btn-si-save')?.addEventListener('click', async () => {
    ...
    try {
      await stockIn({
        ...
        productId,
        quantity: qty,
        costPrice: parseFloat($('#si-cost')?.value) || product.costPrice,
        note: $('#si-note')?.value.trim() || '',
        employeeId: getCurrentEmployee()?.id,
        userId: getCurrentUser()?.uid
      });
      hideLoading();
      showToast('รับเข้าสำเร็จ', 'success');
      openProductDetail(productId);
    } catch (err) {
      hideLoading();
      showToast(err.message || 'ไม่สำเร็จ', 'error');
    }
  });
}
...
export async function openAdjustStockForm(productId) {
  ...
  pageContent.innerHTML = `
    <div class="card">
      <h2 style="font-size:1.1rem;">ปรับ Stock</h2>
      <p class="text-muted">${escapeHtml(product.name)} · คงเหลือปัจจุบัน ${product.stock || 0}</p>
      <div class="form-group"><label>จำนวนใหม่ *</label>
        <input type="number" id="adj-stock" class="form-control" min="0" step="1" value="${product.stock || 0}" inputmode="numeric">
      </div>
      <div class="form-group"><label>เหตุผล *</label>
        <input type="text" id="adj-reason" class="form-control" placeholder="เช่น นับสต็อก / ของเสีย">
      </div>
      <button class="btn btn-primary btn-block" id="btn-adj-save">บันทึก</button>
      <button class="btn btn-outline btn-block mt-1" id="btn-adj-back">กลับ</button>
    </div>`;
...
export async function openStockHistory(productId) {
  ...
  pageContent.innerHTML = `
    <div class="flex-between mb-2">
      ...
      <button class="btn btn-outline btn-sm" id="btn-sh-back">กลับ</button>
    </div>
    ...
    ${!moves.length ? '<div class="card text-muted text-center">ยังไม่มีรายการ</div>' : moves.map(m => {
      ...
      return `<div class="card" style="margin-bottom:8px;padding:10px;">
        <div class="flex-between"><strong>${movementTypeLabel(m.type)}</strong><span>${diff >= 0 ? '+' : ''}${diff}</span></div>
        ...
        ${m.note ? `<div style="font-size:0.85rem;">${escapeHtml(m.note)}</div>` : ''}
      </div>`;
    }).join('')}
  `;
...
export async function renderCategories() {
  ...
  pageContent.innerHTML = `
    <div class="flex-between mb-2">
      ...
      <div style="display:flex;gap:6px;">
        <button class="btn btn-outline btn-sm" id="btn-cat-back">สินค้า</button>
        <button class="btn btn-primary btn-sm" id="btn-cat-add">+ เพิ่ม</button>
      </div>
    </div>
    ${!categories.length ? '<div class="card text-center text-muted">ยังไม่มีหมวดหมู่</div>' : categories.map(c => `
      <div class="card" style="margin-bottom:8px;padding:12px;cursor:pointer;" data-cid="${escapeHtml(c.id)}">
        <strong>${escapeHtml(c.name)}</strong>
        <div class="text-muted" style="font-size:0.8rem;">${c.status === 'ACTIVE' ? 'ใช้งาน' : 'ปิด'} · ลำดับ ${c.sortOrder != null ? c.sortOrder : 0}</div>
      </div>`).join('')}
  `;
  $('#btn-cat-back')?.addEventListener('click', () => renderProducts());
  $('#btn-cat-add')?.addEventListener('click', () => openCategoryForm(null));
  pageContent.querySelectorAll('[data-cid]').forEach(el => {
    el.addEventListener('click', () => openCategoryForm(el.dataset.cid));
  });
}
...
export async function openCategoryForm(categoryId) {
  ...
  pageContent.innerHTML = `
    <div class="card">
      <h2 style="font-size:1.1rem;">${cat ? 'แก้ไขหมวดหมู่' : 'เพิ่มหมวดหมู่'}</h2>
      <div class="form-group"><label>ชื่อ *</label>
        <input type="text" id="c-name" class="form-control" value="${escapeHtml(cat?.name || '')}">
      </div>
      <div class="form-group"><label>ลำดับ</label>
        <input type="number" id="c-sort" class="form-control" value="${cat?.sortOrder != null ? cat.sortOrder : 0}" step="1">
      </div>
      ...
      <button class="btn btn-outline btn-block mt-1" id="btn-c-back">กลับ</button>
    </div>`;
