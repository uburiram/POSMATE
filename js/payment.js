...
/**
 * สร้าง Payload PromptPay (Thai QR Payment)
 * @param {string} promptPayId - เบอร์โทร (0xxxxxxxxx) หรือเลขบัตร 13 หลัก
 * @param {number} amount - ยอดเงิน (ทศนิยม 2 ตำแหน่ง)
...
export function buildPromptPayPayload(promptPayId, amount) {
  ...
  if (target.length >= 13 && target.length <= 15 && !target.startsWith('0')) {
    ...
    // Mobile phone → 0066 + number without leading 0
    if (target.startsWith('0')) target = target.slice(1);
    if (target.startsWith('66')) {
      // already country code style
    } else {
      target = '66' + target;
    }
  ...
  payload += tlv('01', amount != null && amount > 0 ? '12' : '11'); // Dynamic if amount
  payload += tlv('29', merchantInfo); // Merchant Account Information (PromptPay)
